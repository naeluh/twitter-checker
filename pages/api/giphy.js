...
export default async function getGifs(req, res) {
  if (req.method !== 'GET') {
  ...
  const response = await fetch(
    `https://api.giphy.com/v1/gifs/search?api_key=${process.env.GIPHY_API_TOKEN}&q=donaldtrump&limit=100&offset=0&rating=G&lang=${LANG}`
  );
